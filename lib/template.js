...
class Template {
  ...
  init (target, opts) {
    ...
    return this.update().then(() => {
        ...
        return exec('git branch --list', { cwd: this.path }).spread((stdout) => {
          ...
          if (stdout) {
            ...
            if (_.includes(branches, options.branch)) {
              this.sprout.emit('cmd', `git checkout ${options.branch}`, this.path)
      ...
      return W.promise((resolve, reject) => {
        ...
        if (options.config) {
          this.sprout.emit('msg', `merging config: ${options.config}`)
          try {
  ...
  remove () {
    ...
    if (!fs.existsSync(this.path)) { return W.resolve() }
