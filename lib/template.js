...
function mergeConfig (opts) {
  ...
  if (opts.config) {
    ...
    if (/\.json$/.test(opts.config)) {
      externalConfig = require(opts.config)
    } else {
      externalConfig = yaml.safeLoad(fs.readFileSync(opts.config, 'utf8'))
    }
...
function runPrompts (opts) {
  if (lodash.isFunction(opts.questionnaire) && lodash.isArray(this.init.configure)) {
    this.sprout.emit('msg', 'running questionnaire function')
    return opts.questionnaire(this.init.configure, lodash.keys(this.config))
